...
const schema = yup.object({
  ...
  peakHourPrice: yup.number().min(0, 'Price must be positive').nullable().optional(),
  weekendPrice: yup.number().min(0, 'Price must be positive').nullable().optional(),
  minPlayers: yup.number().min(1, 'Minimum 1 player').required('Minimum players is required'),
  maxPlayers: yup.number().min(1, 'Minimum 1 player').required('Maximum players is required'),
  length: yup.number().min(1, 'Length must be positive').required('Length is required'),
  width: yup.number().min(1, 'Width must be positive').required('Width is required'),
}).required();
...
const AddCourtFormV2: React.FC<AddCourtFormProps> = ({ onSubmit, isLoading = false, error }) => {
  ...
  const handleFormSubmit = (data: Record<string, unknown>) => {
    ...
    const courtData: CourtData = {
      ...
      pricing: {
        ...
        peakHourPrice: data.peakHourPrice as number || undefined,
        weekendPrice: data.weekendPrice as number || undefined,
        currency: 'INR',
        hourlyRate: true,
      },
      amenities: amenities.filter(amenity => amenity.trim()), // Filter out empty amenities
      capacity: {
        ...
        <Card>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <Input
              ...
              type="number"
              error={errors.peakHourPrice?.message}
              placeholder="750"
              {...register('peakHourPrice', { valueAsNumber: true })}
            />
